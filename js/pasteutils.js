function UpdatePaste(){
		var respond;
		var Session=UpdatePaste_getCookie("sess");
		var Pass=document.getElementById("pasteInput").value;
		var url="https://api-openid.agatha.org.cn/paste/setPaste";
		var httpRequest = new XMLHttpRequest();//第一步：创建需要的对象
		httpRequest.open('POST' , url , true); //第二步：打开连接
		httpRequest.setRequestHeader("Content-type","application/x-www-form-urlencoded");//设置请求头 注：post方式必须设置请求头（在建立连接后设置请求头）
		httpRequest.send('sess=' + Session+'&content='+Pass);//发送请求 将情头体写在send中
		/**
		 * 获取数据后的处理程序
		 */
...
function UpdatePaste_getCookie(name){
