function ShowSMS(name){
        var Session=ShowSMS_getCookie("sess");
		httpRequest3 = new XMLHttpRequest();//第一步：创建需要的对象
		var url="https://api-openid.agatha.org.cn/sms/getStatus";
		...
		httpRequest3.onreadystatechange = function () {//请求后的回调接口，可将请求成功后要执行的程序写在其中
			if (httpRequest3.readyState == 4 && httpRequest3.status == 200) {//验证请求是否发送成功
				...
				if(respond3.status=="ok"){
                                          document.getElementById("S_Bind").innerHTML="<a class=\"text-black-50\" style=\"margin-right:1rem\">手机号码</a>"+respond3.phone;
				}else {
                                          
                                          document.getElementById("S_Bind").innerHTML="<a class=\"text-black-50\" style=\"margin-right:1rem\">手机号码</a>无数据";
                                }
			}
		};
}

function ShowSMS_getCookie(name){
